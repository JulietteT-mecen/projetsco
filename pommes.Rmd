...
#TAF
<ul>
<li>Faire un trie par profit et faire ACP.</li>
<li>Correlation entre erreur entre production estimée et production réellee en fonction du profit.</li>  
</ul>
...
A faire test de Fisher où $H_0$ est le modele contraint (Cobb Douglass) et $H_1$ non contraint. 
On peut faire une validation descendante pour regarder plus en détail les paramètres. 
...
```{r,warning=FALSE, comment=NA, message=FALSE}
require(stargazer)
data$vCost <- with( data, vLab + vMat)
...
Travail capital
```{r,warning=FALSE, comment=NA, message=FALSE}
lmCES<-lm(log(qCap/qLab)~ log(pLab/pCap))
stargazer(lmCES,type='text')
```

Matériel et travail
```{r,warning=FALSE, comment=NA, message=FALSE}
lmCES2<-lm(log(qMat/qLab)~ log(pLab/pMat))
stargazer(lmCES2,type='text')
```

```{r,warning=FALSE, comment=NA, message=FALSE}
lmCES3<-lm(log(qCap/qMat)~ log(pMat/pCap))
stargazer(lmCES3r,type='text')
```